...
export default {
  ...
  computed: {
    isActiveClass() {
      return this.value === this.$parent.$props.modelValue;
    },
  },
  methods: {
  ...
  mounted() {
    if (this.$parent.$options.name !== 'UiButtonGroup') {
      console.warn(
        `Warning: the element ${this.$options.name} must be rendered inside only <UiButtonGroup></UiButtonGroup>`,
      );
    }
  },
};

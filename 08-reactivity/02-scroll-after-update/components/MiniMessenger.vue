<template>
  <main class="mini-messenger">
    <ul class="messages">
      <li ref="li" v-for="message in messages" :key="message.id" class="message">
        {{ message.text }}
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    async nextTick() {
      const lastRefIndex = this.$refs.li.length - 1;
      await nextTick(() => {
        this.$refs.li[lastRefIndex].scrollIntoView({ behavior: 'smooth' });
      });
    },
  },
};
